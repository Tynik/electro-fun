import React from 'react';
import { SvgIcon } from '@mui/material';
import type { SvgIconOwnProps } from '@mui/material';

export const CapacitorIcon = (props: SvgIconOwnProps) => {
  return (
    <SvgIcon {...props}>
      <svg fill="currentColor" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 460 460">
        <g id="XMLID_995_">
          <path
            id="XMLID_1006_"
            d="M420.259,349.062C368.529,300.485,300.961,273.732,230,273.732c-70.961,0-138.53,26.753-190.259,75.329
		l20.536,21.869c43.688-41.025,98.74-63.202,154.723-66.563V460h30V304.368c55.983,3.36,111.035,25.538,154.722,66.563
		L420.259,349.062z"
          />
          <polygon
            id="XMLID_1007_"
            points="410,146 245,146 245,0 215,0 215,146 50,146 50,176 410,176"
          />
        </g>
      </svg>
    </SvgIcon>
  );
};
